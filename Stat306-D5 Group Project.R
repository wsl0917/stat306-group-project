...
sleep_data <- select(sleep_data, -Bedtime, -Wakeup.time,-ID)
head(sleep_data)
tail(sleep_data)
...
#Assumption checking
model1 <- lm(Sleep.efficiency ~ ., data = sleep_data)
summary(model1)
plot(model1)

#Model Selection
s<-regsubsets(Sleep.efficiency ~ Age+Gender+Sleep.duration+Awakenings+Caffeine.consumption+Alcohol.consumption+Smoking.status+Exercise.frequency+Deep.sleep.percentage,
              data = sleep_data, method="exhaustive")
ss<-summary(s)
ss$which
ss$adjr2
ss$cp
ss$rsq
ss$bic
plot(ss$cp, xlab = "Number of Variables", ylab = "Cp",type="l", main="Cp Plot")
abline(0,1)

#Final model
final_model<-lm(Sleep.efficiency ~ Age+Awakenings+Caffeine.consumption+Alcohol.consumption+Smoking.status+Exercise.frequency+Deep.sleep.percentage , data = sleep_data)
summary(final_model)
